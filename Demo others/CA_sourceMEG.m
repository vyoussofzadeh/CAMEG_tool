...
for j = 1:size(files,2)
    path2 = files{1,j}(1:end-10);
    f = dir([path2,'\*_Mind.mat']);
    load ([path2,'\',f.name]);
    if in ==1
        aValue(:,:,j) = Value(idxp,:);
    else
...
if in3 ==1
    ...
    saValue = aValue(:,f1:f2,:);
    Time  = Time(:,f1:f2);
else
    saValue = aValue;
end

msaValue = mean(saValue,3);
figure,
...
if in4 == 1
    ...
    disp('Teens = 2');
    disp('All  = 3');
    in5 = input ('Enter group type? ');
    if in == 1
        if in5 == 1
            save cameg_MEGsource_Kids aValue saValue Time Atlas idxp ROI
        elseif in5 == 2
            save cameg_MEGsource_Teens aValue saValue Time Atlas idxp ROI
        elseif in5 == 3
            save cameg_MEGsource_all aValue saValue Time Atlas idxp ROI
        end
    else
        if in5 == 1
            save cameg_MEGsource_Kids aValue saValue Time Atlas ROI
        elseif in5 == 2
            save cameg_MEGsource_Teens aValue saValue Time Atlas ROI
        elseif in5 == 3
            save cameg_MEGsource_all aValue saValue Time Atlas ROI
        end
